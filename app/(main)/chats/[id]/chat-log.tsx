...
export default function ChatLog({
  ...
  return (
    <StickToBottom
      ...
      initial="smooth"
    >
      <StickToBottom.Content className="mx-auto flex w-full max-w-prose flex-col gap-8">
...
function UserMessage({ content }: { content: string }) {
  return (
    <div className="inline-flex items-center gap-3 bg-[#262626] p-3 rounded-sm">
      <div className="size-4 rotate-45 rounded-sm" />
      <div className="italic text-[#FFFFFF]">{content}</div>
    </div>
  );
}
  ...
  return (
    <div>
      {parts.map((part, i) => (
        <div key={i}>
          {part.type === "text" ? (
            <Markdown className="prose p-3 bg-[#262626] rounded-sm">{part.content}</Markdown>
