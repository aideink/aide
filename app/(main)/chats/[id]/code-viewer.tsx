...
export default function CodeViewer({
  ...
  return (
    <>
      <div className="flex h-16 shrink-0 items-center justify-between border-b border-gray-300 px-4 bg-[#171717]">
        ...
        {layout === "tabbed" && (
          <div className="rounded-lg border-0 border-gray-300 p-1">
            <button
              onClick={() => onTabChange("code")}
              data-active={activeTab === "code" ? true : undefined}
              className="inline-flex h-7 w-16 bg-[#0A0A0A] text-white items-center justify-center rounded text-xs font-medium data-[active]:bg-[#E1AC07] data-[active]:text-white"
            >
              Code
            </button>
            <button
      ...
      <div className="flex items-center justify-between border-t border-gray-300 px-4 py-4">
        <div className="inline-flex items-center gap-2.5 text-sm">
          ...
          <button
            className="inline-flex items-center gap-1 rounded border border-[#E1AC07] px-1.5 py-0.5 text-sm text-[#E1AC07] transition disabled:opacity-50"
        ...
        <div className="flex items-center justify-end gap-3">
          {previousMessage ? (
            <button
              className="text-[#E1AC07]"
              onClick={() => onMessageChange(previousMessage)}
            >
              <ChevronLeftIcon className="size-4" />
            </button>
          ) : (
            <button className="text-[#E1AC07] opacity-25" disabled>
              <ChevronLeftIcon className="size-4" />
            </button>
          )}

          <p className="text-sm text-[#E1AC07]">
            Version <span className="tabular-nums">{currentVersion + 1}</span>{" "}
            <span className="text-[#E1AC07]">of</span>{" "}
